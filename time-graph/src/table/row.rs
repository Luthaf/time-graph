use super::table_cell::{Alignment, TableCell};
use super::{RowPosition, TableStyle};
use std::cmp::max;
...
impl Row {
            ...
            if self.cells.len() > col_idx {
                ...
                for (line_idx, line) in lines.iter_mut().enumerate().take(row_height) {
                    ...
                    if wrapped_cells[col_idx].len() > line_idx {
                        ...
                        let str_width = wrapped_cells[col_idx][line_idx].width();
    ...
    /// Pads a string according to the provided alignment
